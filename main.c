...
/*
zmienne potrzebne do 'splitowania' node
*/
...
const uint32_t INTERNAL_NODE_NUM_KEYS_SIZE = sizeof(uint32_t);              //liczba kluczy, podziałów
const uint32_t INTERNAL_NODE_NUM_KEYS_OFFSET = COMMON_NODE_HEADER_SIZE;     //za headerem 'kladziemy liczbe kluczy'
const uint32_t INTERNAL_NODE_RIGHT_CHILD_SIZE = sizeof(uint32_t);           //rozmiar wskaznika na prawe dziecko
const uint32_t INTERNAL_NODE_RIGHT_CHILD_OFFSET = INTERNAL_NODE_NUM_KEYS_OFFSET + INTERNAL_NODE_NUM_KEYS_SIZE;  
const uint32_t INTERNAL_NODE_HEADER_SIZE = COMMON_NODE_HEADER_SIZE + INTERNAL_NODE_NUM_KEYS_SIZE + INTERNAL_NODE_RIGHT_CHILD_SIZE;
// Wspólny nagłówek (6 bajtów) + Liczba kluczy (4 bajty) + Prawe dziecko (4 bajty) = Razem 14 bajtów.

/*
 * Internal Node Body Layout
 */
const uint32_t INTERNAL_NODE_KEY_SIZE = sizeof(uint32_t);       //
const uint32_t INTERNAL_NODE_CHILD_SIZE = sizeof(uint32_t); // Rozmiar "wskaźnika" na dziecko (czyli po prostu numer strony w pliku bazy danych). Zajmuje 4 bajty.
// Rozmiar jednej kompletnej komórki w tablicy (Wskaźnik 4 bajty + Klucz 4 bajty = Razem 8 bajtów).
// Służy jako mnożnik (wielkość kroku). Jeśli chcemy wejść na indeks 5,przesuwamy się w pamięci o 5 * 8 bajtów.
...
/* Zapowiedzi funkcji dla kompilatora */
...
uint32_t internal_node_find_child(void* node, uint32_t key){
    ...
    //Binary search, szukamy child do ktorego wstawiamy wartosc
    uint32_t min_index=0;
    uint32_t max_index=num_keys; //jest jednen child wiecej niz keys
...
void internal_node_insert(Table* table, uint32_t parent_page_num, uint32_t child_page_num){
    /*
        Dodajemy nowe (child/key pair) do parenta
    */
    ...
    *internal_node_num_keys(parent) = original_num_keys + 1;
    ...
    if (child_max_key > get_node_max_key(table->pager, right_child)) {
        /*zamieniamy prawe dziecko*/
        *internal_node_child(parent, original_num_keys) = right_child_page_num;         //na ostatni indeks wstawiamy prawe dziecko( stary vip)
        *internal_node_key(parent, original_num_keys) = get_node_max_key(table->pager, right_child);  //aktualizujemy key

        //nowy vip (najwieksze wartosci)
        *internal_node_right_child(parent) = child_page_num;

        //tluamczenie - wskaznik na skrajnie prawe dziecko przechowujemy oddzielnie, musimy postepowac inaczej
    }else{
        //robimy miejsce na nowy klucz
...
void create_new_root(Table* table, uint32_t right_child_page_num){
    /*
    Funkcja ta przemianowuje nasz root/korzen, kopiuje zawartosc aktualnego root,
    wkleja to na nowa strone (nasze left child), zmienia root na nowy
    */
    ...
    if (get_node_type(left_child) == NODE_INTERNAL) {
        void* child;
        // Aktualizujemy wskaźniki u wszystkich zwykłych dzieci w tablicy
        for (int i = 0; i < *internal_node_num_keys(left_child); i++) {
            child = get_page(table->pager, *internal_node_child(left_child, i));
            *node_parent(child) = left_child_page_num;
        }
        // Aktualizujemy wskaźnik u VIP-a (prawego dziecka w nagłówku)
...
void leaf_node_split_and_insert(Cursor* cursor, uint32_t key, Row* value){
    void* old_node = get_page(cursor->table->pager, cursor->page_num);          //nasz obecny node
    uint32_t old_max = get_node_max_key(cursor->table->pager, old_node);                              //najwieksza wartosc jaka przechowuje
    uint32_t new_page_num = get_unused_page_num(cursor->table->pager);          //numer nowej pustej strony
    void* new_node = get_page(cursor->table->pager, new_page_num);              //tworzymy miejsce w pamieci na nowy node
    initialize_leaf_node(new_node);                                             //inicjalizujemy go
    *node_parent(new_node) = *node_parent(old_node);                            //nowy node ma taki sam parent jak stary
